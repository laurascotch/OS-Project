...
help: 
	...
	@echo "\t$(BOLD_PINK)./shell -o=\"file.txt\" -e=\"error.txt\" -c=<true/false> -m=<max-output-lenght>$(NC)"
	@echo "I parametri -c e -m sono opzionali:"
	@echo "-  $(GREEN)c (code)$(NC) richiede o meno la stampa del codice di ritorno"
	@echo "-  $(GREEN)m (maxlen)$(NC) fissa il numero massimo di caratteri di output del comando nel file di log"
	@echo "Sono supportate anche le versioni lunghe delle opzioni: --outfile --errfile --code --maxlen"
